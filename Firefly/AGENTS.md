...
When generating or suggesting code, follow these principles:

- **Modular Design**: Code should be separated into reusable, single-responsibility modules or classes. Avoid monolithic structures unless absolutely necessary.
- **Systems-Level Thinking**: Prefer holistic solutions that integrate cleanly with existing architecture rather than isolated patches. New functionality should extend systems coherently, not bolt on ad hoc.
- **Clean Integration**: Avoid "magic numbers" and hidden dependencies. Use configuration resources, constants, or clearly defined interfaces.
- **Consistency**: Match existing naming conventions, directory structures, and patterns outlined in this document.
- **Maintainability**: Prioritize clarity and explicitness over clever one-liners. Future contributors (including AI agents) should be able to read and extend the code easily.
- **Performance Awareness**: Remember this is a real-time platformer targeting 60-120fps. Optimize inner loops and physics updates, but don't prematurely optimize at the cost of clarity.
- **No Patch Fixes**: Avoid hacks or quick fixes. If a problem requires deeper architectural adjustment, suggest that explicitly instead of hiding complexity with a band-aid solution.
- **Typed GDScript**: Use typed GDScript wherever possible for clarity and performance benefits. This includes function signatures, variable declarations, and return types. On rare occasions where dynamic typing is necessary, document the reason clearly. And when working with legacy code that is untyped, prefer adding type hints incrementally rather than rewriting everything at once.
- **Ternary Operators**: Use ternary operators sparingly and only when they enhance readability. Remember, GDScript's ternary syntax is structured as 'true_value if condition else false_value'. If a ternary expression becomes too complex or nested, prefer a standard if-else block for clarity.
- **Formatting**: Utilize tab indentation for code blocks and ensure well spaced out formatting for readability. Ensure you use consistent spacing and seperation of code blocks with one line of whitespace.
- **LOGGER**: A long time ago I started a logging system. DO NOT USE THIS. It is unmaintained, and uses a massive amount of storage. It's not useful for resting due to the volume of data it generates. Instead, use print statements sparingly, and only for critical debugging information. When suggesting new code, avoid adding new print statements unless absolutely necessary for understanding flow or errors. If logging is needed, suggest a lightweight, context-specific logging approach rather than a full logging system.